...
\begin{figure}[H]
	\caption[An Excitatory Synapse]{An Excitatory Synapse. Input spike  $\rightarrow$
		Neurotransmitter 	release (e.g.,
		Glutamate)  $\rightarrow$
		Binds to ion channel
		receptors $\rightarrow$
		Ion channels open $\rightarrow$
		Na+ influx  $\rightarrow$
		Depolarization due to \gls{gls:EPSP}}
	\includegraphics[width=0.9\textwidth]{synapse4}
\end{figure}

\begin{itemize}
	\item The Synapse Doctrine: Synapses are the basis for memory and learning
	\item Hebbian Plasticity: If neuron A repeatedly takes part in firing neuron B, then the synapse from A to B is strengthened
	\item Neurons that fire together wire together!
	\item Long Term Potentiation (LTP):  Experimentally observed increase in synaptic strength
	that lasts for hours or days
	\item Long Term Depression (LTD): Experimentally observed decrease in synaptic strength
	that lasts for hours or days
\end{itemize}

\begin{figure}[H]
	\caption[Spike-Timing Dependent Plasticity (STDP)]{Spike-Timing Dependent Plasticity (STDP)}
