...
\begin{enumerate}
	...
	\begin{align*}
		p(v\vert u;G)=&\frac{p(u\vert \vu ;G) p(v;G)}{p(u;G)}\\
	...
	\begin{align*}
		\vec{\mu_v} =&\frac{\sum_u P[v\vert \vec{u};G]\vec{u}}{\sum_u P[v\vert \vec{u};G]}\\
		\vec{\sigma_v} =&\frac{\sum_u P[v\vert \vec{u};G]\lvert \vec{u}-\vec{\mu_v}\rvert^2 }{\sum_u P[v\vert \vec{u};G]}\\
		\vec{\gamma_v} =&\frac{\sum_u P[v\vert \vec{u};G]}{N_u}
	\end{align*}
\end{enumerate}
...
\begin{align*}
	...
	=& \sum_{i=1}^{M} \vec{e_i} v_i + noise \text{, where $M\ll N$}
\end{align*}
...
\begin{align*}
	\vec{u} =& \sum_{i=1}^{M} \vec{g}_i v_i + noise\\
	=& G \vec{v} + noise
\end{align*} 
...
\begin{align*}
	p[\vec{u}\vert \vec{v};G] =& Gaussian(u;Gv,I)\\
	\propto exp\big(-\frac{\lvert\lvert \vec{u}-G \vec{v}\rvert rvert^2}{2}\big)\text{, whence}\\
	\log p[\vec{u}\vert \vec{v};G] =& -\frac{\lvert\lvert \vec{u}-G \vec{v}\rvert rvert^2}{2} + C
\end{align*}

So minimizing the reconstruction error is the same as maximizing likelihood.

We will assume that the causes are independent, for simplicity, so $P[\vec{v}]$ is product of probabilities of the originak causes.
...
\begin{itemize}
	...
	\item Equivalently, maximize log posterior
	\item Alternate between two steps
	(similar to EM algorithm):
	\begin{itemize}
		\item 	Maximize F with respect to v, 	keeping G fixed
		\item Maximize F with respect to G, given the v from above
	\end{itemize}
\end{itemize}
...
\begin{figure}[H]
	\begin{center}
		\caption[Learning G for Natural Images]{Learning G for Natural Images\cite{olshausen1997sparse}}
...
\begin{figure}[H]
	\begin{center}
		\caption[Sparse Coding Network is a special case of Predictive Coding Networks]{Sparse Coding Network is a special case of Predictive Coding Networks\cite{rao1999optimal,rao1999predictive}}
