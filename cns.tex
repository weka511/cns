...
\begin{align*}
	g_k(V) =& \bar{g}_k n^4 \numberthis \label{eq:gK}\\
	g_{Na}(V)=& \bar{g}_{Na} m^3h \numberthis \label{eq:gNa}
\end{align*}
...
The voltage across the membrane changes as the result of changes in the external driving current, and also because the opening are closing probabilities cause the conductances to vary.
Using Ohm's Law
\begin{align*}
	V =& IR \text{, and Kirchhoff's Law}\\
	\underbrace{C_m\frac{dV}{dt}}_\text{Capacitative current}=&-\underbrace{\sum_{i}g_i(V-E_i)}_\text{ionic currents} + \underbrace{I_e}_\text{external current}\numberthis\label{eq:hodgkin:huxley:precursor}
\end{align*}
Equations \eqref{eq:hodgkin:huxley:precursor} is known as the Hodgkin-Huxley equation\cite{hodgkin1952currents}. Substituting \eqref{eq:gK} and \eqref{eq:gNa}:

\begin{align*}
	-C_m\frac{dV}{dt}=&g_L(V-E_L)+\bar{g_k}n^4(V-E_K)+\bar{g_{Na}}m^3h(V-E_{Na})-I_e\numberthis\label{eq:hodgkin:huxley}
\end{align*}
We use \eqref{eq:n}, \eqref{eq:m}, and \eqref{eq:h} to produce Figure \ref{fig:dynamics:act:inact}.
\begin{figure}[H]
	\caption[Dynamics of Activation Inactivation]{Dynamics of Activation Inactivation. Panel (\subref{fig:dynamics:act:inact2}) is based on using $\tau_n(V) \frac{dn}{dt} = n_\infty(V)-n$}\label{fig:dynamics:act:inact}
	\begin{subfigure}[t]{0.45\textwidth}
		\caption{Where each variable is going}\label{fig:dynamics:act:inact2}
		\includegraphics[width=\textwidth]{dynamics-activation-inactivation1}
	\end{subfigure}	
	\begin{subfigure}[t]{0.45\textwidth}
		\caption{How quickly each variable responds to a change in voltage.}\label{fig:dynamics:act:inact2}
		\includegraphics[width=\textwidth]{dynamics-activation-inactivation2}
	\end{subfigure}
\end{figure}

Recall that $n$ governs the opening of the potassium channel, and $m$ and $h$ have to be large to open the sodium channel. From \ref{fig:dynamics:act:inact2} we see that the probability of the Potassium channel opening increases with voltage, but the probability of the sodium channel opening first increases $m^3$ then decreases $h$, since $n$ is a sigmoid.

If we change $V$, the steady state changes, and Figure \ref{fig:dynamics:act:inact2} tells us how quickly the variables change to the new steady state. Since $\tau_m$ has the smallest value, $m$ will react fastest.

\begin{figure}[H]
	\caption[Anatomy of a Spike]{Anatomy of a Spike. notice the resting potentials in the top top left sub-panel. The potassium channel tries to move the potential to $E_K$, and the sodium channel to $E_{Na}$. Imagine that the neuron is sitting at about -60mV, and an input comes along that depolarizes the membrane. Since $m$ has the shortest time constant, it will adjust to the new voltage first. The sodium channel opens, and moves the equilibrium towards $E_{Na}$, creating a positive feedback. Next $h$ starts to decrease, which tends to close the sodium channel, and finally $n$ starts to open the potassium channel.}
